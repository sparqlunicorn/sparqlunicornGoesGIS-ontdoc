    ...
    def generateOntDocForNameSpace(self, prefixnamespace,dataformat="HTML"):
        ...
        if os.path.exists(outpath + self.pubconfig["corpusid"] + '_classtree.js'):
            try:
                ...
                print("Exception occurred " + str(e))
        classidset = set()
        rescls = ClassTreeUtils.getClassTree(self.graph, uritolabel, classidset, uritotreeitem,self.typeproperty,self.pubconfig["prefixes"],self.preparedclassquery)
        tree=rescls[0]
        uritotreeitem=rescls[1]
        #print(str(tree))
        for tr in prevtree:
