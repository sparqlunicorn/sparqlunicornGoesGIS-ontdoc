...
class OntDocGeneration:
    ...
    def getClassTree(self, graph, uritolabel, classidset, uritotreeitem):
        ...
        for cls in ress:
            ...
            if ress[cls]["super"] == None:
                ...
                if cls not in uritotreeitem:
                    result.append(
                        {"id": cls, "parent": ress[cls]["super"], "type": "class", "text": restext, "data": {}})
                ...
                if str(ress[cls]["super"]) not in uritotreeitem:
                    ...
                    if clsres != None:
                        ...
                        theitem = {"id": str(ress[cls]["super"]), "parent": "#", "type": "class",
                                   "text": DocUtils.shortenURI(str(ress[cls]["super"])), "data": {}}
        ...
        for tup in graph.predicate_objects(object):
            if str(tup[0]) in DocConfig.labelproperties:
                if tup[1].language == self.labellang:
                    label = str(tup[1])
                onelabel = str(tup[1])
            if pred == "http://www.w3.org/ns/oa#hasSelector" and tup[0] == URIRef(self.typeproperty) and (
            ...
            if (
                    ...
                    tup[0] == URIRef(self.typeproperty) and ("http://purl.org/ontology/bibo/" in str(tup[1])):
    ...
    def createHTMLTableValueEntry(self, subject, pred, object, ttlf, graph, baseurl, checkdepth, geojsonrep, foundmedia,
                                  imageannos, textannos, image3dannos, annobodies, dateprops, inverse, nonns):
        tablecontents = ""
        label = ""
        bibtex = None
        timeobj = None
        if isinstance(object, URIRef) or isinstance(object, BNode):
            ...
            label = ""
            unitlabel = ""
            mydata = self.searchObjectConnectionsForAggregateData(graph, object, pred, geojsonrep, foundmedia,
                                                                  imageannos, textannos, image3dannos, annobodies,
                                                                  label, unitlabel, nonns, inverse)
            label = mydata["label"]
            if label == "":
            ...
            if baseurl in str(object) or isinstance(object, BNode):
                ...
                tablecontents += "<span><a itemprop=\"" + str(pred) + "\"" + microdatares + "property=\"" + str(
                    pred) + "\" " + rdfares + " href=\"" + rellink + "\">" + label + " <span style=\"color: #666;\">(" + self.namespaceshort + ":" + str(
            ...
            if isinstance(object, Literal) and object.datatype != None:
                ...
                if res != None:
                    tablecontents += "<span itemprop=\"" + str(pred) + "\" property=\"" + str(
                        ...
                        object.datatype) + "\">" + self.truncateValue(
                ...
                if object.language != None:
                    tablecontents += "<span itemprop=\"" + str(pred) + "\" property=\"" + str(
                        ...
                        object.language) + "\">" + self.truncateValue(str(object).replace("<", "&lt").replace(">",
    ...
    def createHTML(self, savepath, predobjs, subject, baseurl, subpreds, graph, searchfilename, classtreename,
        ...
        if uritotreeitem != None and str(subject) in uritotreeitem and uritotreeitem[str(subject)][-1][
                ...
                if len(predobjmap[tup]) > 0:
                    ...
                    for item in predobjmap[tup]:
                        ...
                        res = self.createHTMLTableValueEntry(subject, tup, item, ttlf, graph,
                                                             ...
                                                             textannos, image3dannos, annobodies, dateprops, inverse,
                                                             nonns)
                ...
                if len(subpredsmap[tup]) > 0:
                    ...
                    for item in subpredsmap[tup]:
                        ...
                        res = self.createHTMLTableValueEntry(subject, tup, item, None, graph,
                                                             baseurl, checkdepth, geojsonrep, foundmedia, imageannos,
                                                             textannos, image3dannos, annobodies, None, True, nonns)
                        foundmedia = res["foundmedia"]
                        imageannos = res["imageannos"]
                        image3dannos = res["image3dannos"]
                        annobodies = res["annobodies"]
            ...
            with open(completesavepath, 'w', encoding='utf-8') as f:
                    ...
                    if self.iiif:
                        iiifmanifestpaths["default"].append(
                            IIIFAPIExporter.generateIIIFManifest(graph, self.outpath, self.deploypath,
                                                                 ...
                                                                 str(subject), self.prefixnamespace, imagetoURI,
                ...
                if len(imageannos) > 0 and len(foundmedia["image"]) > 0:
                    ...
                    for image in foundmedia["image"]:
                        ...
                        if "<svg" in image:
                            if "<svg>" in image:
                                f.write(templates["imagestemplatesvg"].replace("{{carousel}}", carousel).replace(
