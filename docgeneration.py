...
class OntDocGeneration:
    ...
    def generateOGCAPIFeaturesPages(self,outpath,featurecollectionspaths,prefixnamespace,ogcapi,mergeJSON):
        if ogcapi:
            apihtml="<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"utf-8\"><title>SwaggerClient for API "+str(self.deploypath)+"</title><script src=\"https://unpkg.com/swagger-client\"></script>  <script>new SwaggerClient('"+str(self.deploypath)+"/api/index.json').then(client => client.apis.pet.addPet({ id: 1, body: { name: \"bobby\" } }),reason => console.error('failed to load the spec: ' + reason)).then(addPetResult => console.log(addPetResult.body),reason => console.error('failed on api call: ' + reason));</script></head><body>check console in browser's dev. tools</body></html>"
        ...
        for coll in featurecollectionspaths:
            ...
            if ogcapi:
                ...
                apijson["paths"]["/collections/"+str(opweb.replace(".geojson",""))]={"get":{"tags":["Collections"],"summary": "describes collection {collectionId}","description": "Describes the collection with the id {collectionId}","operationId": "collection-umring","parameters":[],"responses": {"default": {"description": "default response","content": {"application/json": {"schema": {"$ref": "#/components/schemas/Collections"},"example": None}}}}}}
