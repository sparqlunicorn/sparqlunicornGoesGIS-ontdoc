...
class OntDocGeneration:
    ...
    def generateOGCAPIFeaturesPages(self,outpath,featurecollectionspaths,prefixnamespace,ogcapi,mergeJSON):
            ...
            if ogcapi:
                ...
                if os.path.exists(coll):
                    try:
                        ...
                        p.symlink_to(targetpath)
                        targetpath=self.generateRelativeSymlink(coll.replace("//","/").replace("index.geojson","index.xhtml"),str(op+"/items/index.html").replace("//","/"),outpath)
                        f=open(str(op+"/items/index.xhtml"),"w")
                        ...
                        print(e)
                    apijson["paths"][str("/collections/"+str(coll.replace(outpath,"").replace("index.geojson","").replace(".geojson","")[1:])+"/items/index.json").replace("//","/")]={"get":{"tags":["Data"]},"summary": "retrieves features of collection "+str(coll.replace(outpath,"").replace("index.geojson","").replace(".geojson","")[1:]).rstrip("/"),"description": "Retrieves features of collection  "+str(coll.replace(outpath,"").replace("index.geojson","").replace(".geojson","")[1:]),"operationId": "feature-"+str(coll.replace(outpath,"").replace("index.geojson","").replace(".geojson","")[1:]),"parameters":[{"name":"featureId","in": "path","required": True,"schema": {"type": "string"}}],"responses": {"default": {"description": "default response","content": {"application/geo+json": {"example": None }},"text/ttl": {"schema": {"example": None},"example": None},"text/html": {"schema": {"example": None},"example": None}}}}
                    ...
                    for feat in curcoll["features"]:
                        ...
                        try:
                            ...
                            p.symlink_to(targetpath)
                            targetpath=self.generateRelativeSymlink(featpath+"/index.xhtml",str(op+"/items/"+str(self.shortenURI(feat["id"]))+"/index.html").replace("//","/"),outpath)
                            f=open(str(op+"/items/"+str(self.shortenURI(feat["id"])))+"/index.xhtml","w")
                            f.write("<html><head><meta http-equiv=\"refresh\" content=\"0; url="+targetpath+"\" /></head></html>")
                            f.close()
        ...
        if ogcapi:
            ...
            f.write(apihtml)
            f.close()
            f=open(outpath + "/collections/index.xhtml","w",encoding="utf-8")
